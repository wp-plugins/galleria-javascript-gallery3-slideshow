...
function gallery3galleriaslideshow($atts) {
	...
	#galleria'.$albumnum.' {
		...
		<div class="content">
			<div id="galleria'.$albumnum.'"><img style="position:absolute;top:20px;right:20px;" src="'.$pluginUrl.'/galleria/themes/classic/classic-loader.gif"/></div>
			<div id="theButton'.$albumnum.'" class="play"><a href="#"></a></div>
		</div>
		<script>
			var gallery, images'.$albumnum.' = [], tmpImage, tmpBigImage ;
			jQuery(document).ready(function(){
				Galleria.loadTheme(\''.$pluginUrl.'/galleria/themes/classic/galleria.classic.min.js\');
				getItems'.$albumnum.'(\''.$gallerypath.'index.php/rss/feed/gallery/album/'. $albumnum.'\');
			});
			function getItems'.$albumnum.'(earl'.$albumnum.'){
				jQuery.get(earl'.$albumnum.', {}, function(data){
				var next'.$albumnum.' = jQuery(data).find(\'atom\\\\:link[rel="next"]\').attr(\'href\');
				//console.log(earl);
				...
				if (next'.$albumnum.' != undefined) {
					getItems'.$albumnum.'(next'.$albumnum.');
				} else {
					jQuery(\'#galleria'.$albumnum.'\').galleria({
						data_source: images'.$albumnum.',
						showInfo: '.$showinfo.',
						imageCrop: '.$imagecrop.',
